...
begin
  if MessageDlg('Êtes-vous sûr de vouloir supprimer ce utilisateur ?',
    mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin

    if DM.T_utilisateur.Locate('idutilisateur',
      DM.T_utilisateur.FieldByName('idutilisateur').AsInteger, []) then
    begin
      DM.T_utilisateur.Delete;
      ShowMessage('L''utilisateur été supprimé.');
    end
    else
      ShowMessage('L''utilisateur n''a pas été trouvé.');
  end;
